...
(   
    ...
    output reg bus_we,
    input bus_rdy,
    input [255:0] i_bus_data,
    output [255:0] o_bus_data,
...
//reg [2:0] j,i;
//reg [1:0] k;
reg [31:0] i__store,j__store,k__store;
//reg [2:0] i__store, j__store;
//reg [2:0] i__store_buff, j__store_buff;
//reg [1:0] k__store;
//reg [1:0] k__store_buff;
reg [31:0] i__store_buff,j__store_buff,k__store_buff;
...
assign read_buff = (write_buff_int & ((load_mask[i]) << (j << 5) << (k << 3))) | (i_bus_data & ~((load_mask[i]) << (j << 5) << (k << 3))); 
...
always @(posedge clk) begin
    if(rst)
        ...
        proc_data_buff <= (i_bus_data >> (j__store << 5)) >> (k__store << 3); 
end
...
always @(posedge clk) begin
	...
	Store_LRU_Read_Data_Buffer <= 2'b00;
    end
    else begin
...
assign o_bus_data = ((state == REPLACE) | ((state == WAIT) & (bus_data_en | bus_data_en_reg))) ? 
...
always @(posedge clk) begin
    ...
    else if(cache_flush_int & (state == REPLACE)) begin
...
always @(posedge clk) begin
    if(rst) begin
        ...
        if(Read__reg) begin
            DCache_Write_Data_w0_temp <= i_bus_data;
            DCache_Write_Data_w1_temp <= i_bus_data;
        end
        else begin 
...
always @(*) begin
    ...
    if(Tag_Addr_Counter == 7'b1111111) begin
        Tag_Clear_Done = 1'b1;   
    end
    else Tag_Clear_Done = 1'b0;
end
...
always @(*) begin
    case(state)
            ...
            if(~cache_flush_int) begin  
                ...
                if(hit & Read__reg & ~prp_acs_int) begin
                    ...
                    if (Load_Store__req & Read) begin
                        ...
                        //DCache_Write_En_w0 = (tag_comp_w0 && ~addr_exception) ? ((store_map[i__store] << (4))) : 32'b0;   
        ...
        TAG_CLEAR : begin
            ...
            Tag_Write_En_w0 = (tag_clear) ? 4'b1111 : 4'b0000;
            Tag_Write_En_w1 = (tag_clear) ? 4'b1111 : 4'b0000;
            Dirty_bit_Write_En_w0 = 1'b0;
            Dirty_bit_Write_En_w1 = 1'b0;
